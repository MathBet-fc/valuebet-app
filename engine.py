...
# Percorso del file storico robusto rispetto alla directory di lavoro
_HISTORY_PATH = os.path.join(os.path.dirname(os.path.abspath(__file__)), 'mathbet_history.json')

# ==============================================================================
# FUNZIONI DI SUPPORTO (estratte come funzioni di modulo)
# ==============================================================================
def _calculate_segment(hist_list):
    """Calcola le statistiche aggregate per un segmento di partite (totale, forma, casa, trasferta)."""
    m = len(hist_list)
    if m == 0:
        return {"goals_total": 0, "ga_total": 0, "xg_total": 0, "xga_total": 0,
                "npxg_total": 0, "npxga_total": 0, "ppda": 12.0, "oppda": 12.0,
                "dc": 5.0, "odc": 5.0, "pts": 0, "xpts": 0, "matches": 0}
...
def get_team_metric(team_stats, segment, metric, data_mode):
    """Estrae il valore medio per match di attacco (gf) o difesa (gs) per un dato segmento e modalità dati."""
    ...
    if data_mode == "Solo Gol Reali":
        return v_goals / matches
    elif data_mode == "Solo xG (NPxG Mode)":
        return v_npxg / matches
    else:
        return ((v_npxg * 0.40) + (v_xg * 0.30) + (v_goals * 0.30)) / matches


def get_blended_val(stats_dict, metric, data_mode, w_seas):
    """Mix ponderato tra statistiche di stagione e trend forma."""
    val_tot = get_team_metric(stats_dict, 'total', metric, data_mode)
    val_form = get_team_metric(stats_dict, 'form', metric, data_mode)
    ...
    try:
        with UnderstatClient() as client:
            ...
            for team_id, details in data.items():
                ...
                stats_db[t_name] = {
                    "total": _calculate_segment(hist),
                    "form": _calculate_segment(hist[-5:]),
                    "home": _calculate_segment([m for m in hist if m.get('h_a') == 'h']),
                    "away": _calculate_segment([m for m in hist if m.get('h_a') == 'a'])
                }
...
# ==============================================================================
# ⚙️ CALCOLO xG CONDIVISO (elimina duplicazione tra app.py e find_top_value_bets)
# ==============================================================================
def calculate_match_xg(h_stats, a_stats, L_DATA, m_type, h_elo, a_elo, w_seas, data_mode,
                        h_att=None, h_def=None, a_att=None, a_def=None,
                        h_att_split=None, h_def_split=None, a_att_split=None, a_def_split=None,
                        volatility=1.0, h_str=100, a_str=100, h_rest=7, a_rest=7,
                        is_big_match=False, h_m_a=False, h_m_d=False, a_m_a=False, a_m_d=False):
    """
    Calcola gli xG finali aggiustati per entrambe le squadre.
    Se h_att/a_att etc. non vengono passati, li calcola automaticamente dalle statistiche.
    Restituisce (f_xh, f_xa, h_ppda, a_ppda, h_dc, a_dc).
    """
    w_split = 0.60

    # Estrai statistiche di attacco/difesa se non fornite esternamente
    if h_att is None:
        h_att = get_blended_val(h_stats, 'gf', data_mode, w_seas)
    if h_def is None:
        h_def = get_blended_val(h_stats, 'gs', data_mode, w_seas)
    if a_att is None:
        a_att = get_blended_val(a_stats, 'gf', data_mode, w_seas)
    if a_def is None:
        a_def = get_blended_val(a_stats, 'gs', data_mode, w_seas)

    # Statistiche casa/trasferta
    if h_att_split is None:
        h_att_split = get_team_metric(h_stats, 'home', 'gf', data_mode) if h_stats['home']['matches'] > 0 else h_att
    if h_def_split is None:
        h_def_split = get_team_metric(h_stats, 'home', 'gs', data_mode) if h_stats['home']['matches'] > 0 else h_def
    if a_att_split is None:
        a_att_split = get_team_metric(a_stats, 'away', 'gf', data_mode) if a_stats['away']['matches'] > 0 else a_att
    if a_def_split is None:
        a_def_split = get_team_metric(a_stats, 'away', 'gs', data_mode) if a_stats['away']['matches'] > 0 else a_def

    h_fin_att = (h_att * (1 - w_split)) + (h_att_split * w_split)
    h_fin_def = (h_def * (1 - w_split)) + (h_def_split * w_split)
    a_fin_att = (a_att * (1 - w_split)) + (a_att_split * w_split)
    a_fin_def = (a_def * (1 - w_split)) + (a_def_split * w_split)
    ...
    # Correzione pts vs xpts (fortuna/sfortuna)
    ...
    # Regressione alla media (goal diff estremi)
    ...
    return f_xh, f_xa, h_ppda, a_ppda, h_dc, a_dc
...
def find_top_value_bets(all_odds, stats_db, L_DATA, volatility, m_type, elo_dict, w_seas, data_mode, ml_models=None):
    ...
    for match in all_odds:
        ...
        # Usa la funzione condivisa (nessuna duplicazione)
        f_xh, f_xa, h_ppda, a_ppda, h_dc, a_dc = calculate_match_xg(
            h_stats, a_stats, L_DATA, m_type, h_elo, a_elo, w_seas, data_mode,
