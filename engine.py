            ...
            if match['home_team'] == matched_home:
                ...
                for m in bookie['markets']:
                    if m['key'] == 'h2h':
                        ...
                        outcome_map = {o.get('name'): o.get('price') for o in outcomes if o.get('name') is not None}
                        names = list(outcome_map.keys())

                        matched_home_out = difflib.get_close_matches(home_team, names, n=1, cutoff=0.45)
                        matched_away_out = difflib.get_close_matches(away_team, names, n=1, cutoff=0.45)

                        # Draw spesso è "Draw" o "X" a seconda del feed
                        draw_out = next(
                            (n for n in names if str(n).lower() in ("draw", "d") or str(n).strip().upper() == "X" or "draw" in str(n).lower()),
                            None,
                        )
                        if draw_out is None:
                            m_draw = difflib.get_close_matches("Draw", names, n=1, cutoff=0.35)
                            draw_out = m_draw[0] if m_draw else None

                        b1 = outcome_map.get(matched_home_out[0], 0) if matched_home_out else 0
                        b2 = outcome_map.get(matched_away_out[0], 0) if matched_away_out else 0
                        bX = outcome_map.get(draw_out, 0) if draw_out else 0

                        # Garantiamo sempre numeri (evita crash in app.py)
                        b1 = float(b1) if b1 is not None else 0.0
                        b2 = float(b2) if b2 is not None else 0.0
                        bX = float(bX) if bX is not None else 0.0

                        # Chiavi allineate a home_team/away_team passati e a "Draw"
                        odds_data['h2h'] = {home_team: b1, "Draw": bX, away_team: b2}
                    elif m['key'] == 'totals':
                        o25 = [
                            o.get('price')
                            for o in m.get('outcomes', [])
                            if o.get('point') == 2.5 and "over" in str(o.get('name', "")).lower() and o.get('price') is not None
                        ]
                        u25 = [
                            o.get('price')
                            for o in m.get('outcomes', [])
                            if o.get('point') == 2.5 and "under" in str(o.get('name', "")).lower() and o.get('price') is not None
                        ]
                        if o25 and u25: 
...
def infer_max_goals(f_xh: float, f_xa: float, min_goals: int = 10, extra: int = 7, max_cap: int = 25) -> int:
    """
    Sceglie dinamicamente la dimensione della matrice (N x N) per limitare il truncamento.
    """
    try:
        mu_h = max(0.01, float(f_xh))
        mu_a = max(0.01, float(f_xa))
        est = int(max(mu_h, mu_a) + extra)
        return int(max(min_goals, min(est, max_cap)))
    except Exception:
        return min_goals

def build_dixon_coles_probabilities(
    f_xh: float,
    f_xa: float,
    rho: float,
    max_goals: int,
    ...
    """
    Crea la matrice delle probabilita' punteggio (Dixon-Coles) e aggrega:
    - 1 / X / 2
    - BTTS (pGG)
    - Over 2.5 (pO25)
    """
    mu_h = max(0.01, float(f_xh))
    mu_a = max(0.01, float(f_xa))
    max_k = max(1, int(max_goals))

    # Poisson pmf via ricorrenza (piu' veloce e stabile del factorial)
    pmf_h = np.zeros(max_k + 1, dtype=float)
    pmf_a = np.zeros(max_k + 1, dtype=float)
    pmf_h[0] = math.exp(-mu_h)
    pmf_a[0] = math.exp(-mu_a)
    for k in range(max_k):
        pmf_h[k + 1] = pmf_h[k] * mu_h / (k + 1)
        pmf_a[k + 1] = pmf_a[k] * mu_a / (k + 1)

    matrix = np.outer(pmf_h, pmf_a)

    # Correzione Dixon-Coles solo per punteggi bassi
    if max_k >= 1:
        matrix[0, 0] *= (1.0 - (mu_h * mu_a * rho))
        matrix[0, 1] *= (1.0 + (mu_h * rho))
        matrix[1, 0] *= (1.0 + (mu_a * rho))
        matrix[1, 1] *= (1.0 - rho)

    # La vecchia versione clippava ogni singola cella; manteniamo lo stesso comportamento
    matrix = np.maximum(0.0, matrix)

    idx_h, idx_a = np.indices(matrix.shape)
    p1 = float(matrix[idx_h > idx_a].sum())
    pX = float(matrix[idx_h == idx_a].sum())
    p2 = float(matrix[idx_h < idx_a].sum())
    pGG = float(matrix[(idx_h > 0) & (idx_a > 0)].sum())
    pO25 = float(matrix[(idx_h + idx_a) > 2.5].sum())

    tot = float(matrix.sum())
    if tot > 0:
        matrix /= tot
        p1 /= tot
        pX /= tot
    ...
    if return_matrix:
        return matrix, p1, pX, p2, pGG, pO25
...
def monte_carlo_simulation(f_xh, f_xa, n_sims=5000):
    # Vectorizzazione: molto piu' veloce del loop e non genera una lista enorme.
    mu_h = max(0.1, float(f_xh))
    mu_a = max(0.1, float(f_xa))

    lambda_h = np.clip(np.random.normal(mu_h, 0.20 * mu_h, size=n_sims), 0.1, None)
    lambda_a = np.clip(np.random.normal(mu_a, 0.20 * mu_a, size=n_sims), 0.1, None)

    gh = np.random.poisson(lambda_h)
    ga = np.random.poisson(lambda_a)

    outcomes = np.where(gh > ga, 1, np.where(gh == ga, 0, 2))  # 1=Casa, 0=Draw, 2=Ospite
    counts = np.bincount(outcomes, minlength=3)
    return counts, int(n_sims)
...
def find_top_value_bets(all_odds, stats_db, L_DATA, volatility, m_type, elo_dict, w_seas, data_mode, ml_models=None):
    ...
    for match in all_odds:
        ...
        try:
            ...
            for m in bookie['markets']:
                if m['key'] == 'h2h':
                    ...
                    home_out = difflib.get_close_matches(h_name, names, n=1, cutoff=0.45)
                    away_out = difflib.get_close_matches(a_name, names, n=1, cutoff=0.45)
                    ...
                    b1 = outcome_map.get(home_out[0], 0) if home_out else 0
                    b2 = outcome_map.get(away_out[0], 0) if away_out else 0
                    bX = outcome_map.get(draw_out, 0) if draw_out else 0
                    ...
                    bO25 = float(o25[0]) if o25 else 0.0
        except: 
            continue
        ...
        max_goals = infer_max_goals(f_xh, f_xa)
        p1, pX, p2, pGG, pO25 = build_dixon_coles_probabilities(
            f_xh, f_xa, L_DATA["rho"], max_goals, return_matrix=False
        )
