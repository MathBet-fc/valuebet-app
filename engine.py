...
# --- FIX SELENIUM 4.29+ / PYTHON 3.13 PER UNDERSTATAPI ---
try:
    import selenium.webdriver.chrome.options
    import selenium.webdriver.firefox.options
    def _fake_set_headless(self, headless=True):
        if headless: self.add_argument('--headless')
    selenium.webdriver.chrome.options.Options.set_headless = _fake_set_headless
    selenium.webdriver.firefox.options.Options.set_headless = _fake_set_headless
except Exception as e:
    import streamlit as st
    st.error(f"Monkey patch fallito: {e}")
# ---------------------------------------------------------
