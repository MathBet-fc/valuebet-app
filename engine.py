...
# ==============================================================================
# CONFIGURAZIONE COSTANTI E DATA EXTRACTION
# ==============================================================================
LEAGUES_CONFIG = {
    ...
    try:
        with UnderstatClient() as client:
            ...
            def calculate_segment(hist_list):
                ...
                for h in hist_list:
                    goals += h.get('scored', 0); ga += h.get('missed', 0)
                    xg += h.get('xG', 0); xga += h.get('xGA', 0)
                    npxg += h.get('npxG', h.get('xG', 0)); npxga += h.get('npxGA', h.get('xGA', 0))
                    dc += h.get('deep', 5); odc += h.get('deep_allowed', 5)
                    pts += h.get('pts', 0); xpts += h.get('xpts', 0)
                    p_def = h.get('ppda', {}).get('def', 0)
                    ppda_sum += (h.get('ppda', {}).get('att', 0)/p_def) if p_def > 0 else 12.0
                    op_def = h.get('ppda_allowed', {}).get('def', 0)
                    oppda_sum += (h.get('ppda_allowed', {}).get('att', 0)/op_def) if op_def > 0 else 12.0
                return {"goals_total": goals, "ga_total": ga, "xg_total": xg, "xga_total": xga, "npxg_total": npxg, "npxga_total": npxga, "ppda": ppda_sum / m, "oppda": oppda_sum / m, "dc": dc / m, "odc": odc / m, "pts": pts, "xpts": xpts, "matches": m}
...
# ==============================================================================
# 🤖 MACHINE LEARNING (MULTI-TARGET DAI RISULTATI ESATTI)
# ==============================================================================
def train_ml_models(history_data):
    ...
    if len(X) < 15: return None # Sicurezza anti-overfitting
...
def apply_ml_boost(models_dict, f_xh, f_xa, p1_stat, px_stat, p2_stat, po25_stat, pgg_stat, h_elo, a_elo, h_ppda, a_ppda, h_dc, a_dc, w_seas, volatility):
    ...
    probs_1x2 = m_1x2.predict_proba(features)[0]; classes_1x2 = list(m_1x2.classes_)
    ...
    probs_uo25 = m_uo25.predict_proba(features)[0]; classes_uo25 = list(m_uo25.classes_)
    ml_po25 = probs_uo25[classes_uo25.index(1)] if 1 in classes_uo25 else 0.0
    
    m_btts = models_dict["btts"]
    probs_btts = m_btts.predict_proba(features)[0]; classes_btts = list(m_btts.classes_)
...
# ==============================================================================
# GESTIONE STORICO ED EXPORT (LOCAL JSON CON FUNZIONI DI BACKUP)
# ==============================================================================
