...
def _get_understat_json(url):
    res = requests.get(url, headers={'X-Requested-With': 'XMLHttpRequest'}, timeout=15)
    return res.json()
...
def fetch_understat_data_auto(league_name):
    ...
    try:
        # Algoritmo stagione: da Agosto in poi la stagione inizia col nuovo anno, sennò l'anno precedente
        curr_y = datetime.now().year
        u_season = curr_y if datetime.now().month >= 8 else curr_y - 1
        url = f"https://understat.com/getLeagueData/{u_league}/{u_season}"
        raw = _get_understat_json(url)
        data = raw.get("teams", {})
        if not data: return {}, []
        stats_db = {}
        team_list = []
...
def calculate_live_pressure_index(stats_home, stats_away, current_minute):
    ...
    da_h = float(stats_home.get('Dangerous attacks', 0))
    da_a = float(stats_away.get('Dangerous attacks', 0))
    sot_h = float(stats_home.get('Shots on target', 0))
    sot_a = float(stats_away.get('Shots on target', 0))
    c_h = float(stats_home.get('Corner kicks', 0))
    c_a = float(stats_away.get('Corner kicks', 0))
