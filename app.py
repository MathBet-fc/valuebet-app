...
with st.sidebar:
    ...
    with st.expander("🔢 Dati Calci d'Angolo", expanded=False):
        ...
        c1, c2 = st.columns(2)
        corn_f_h = c1.number_input("Fatti Casa", 0.0, 15.0, 5.5, 0.1); corn_s_h = c1.number_input("Subiti Casa", 0.0, 15.0, 4.5, 0.1)
        corn_f_a = c2.number_input("Fatti Ospite", 0.0, 15.0, 4.5, 0.1); corn_s_a = c2.number_input("Subiti Ospite", 0.0, 15.0, 5.5, 0.1)

    with st.expander("🔢 Altri Dati Extra", expanded=False):
        c_card_h = st.number_input("Cart. Casa", 0.0, 10.0, 2.0, 0.1); c_card_a = st.number_input("Cart. Ospite", 0.0, 10.0, 2.5, 0.1)
        c_shot_h = st.number_input("Tiri Casa", 0.0, 50.0, 12.0, 0.5); c_shot_a = st.number_input("Tiri Ospite", 0.0, 50.0, 10.0, 0.5)
        c_sot_h = st.number_input("Tiri Porta C", 0.0, 20.0, 4.5, 0.1); c_sot_a = st.number_input("Tiri Porta O", 0.0, 20.0, 3.5, 0.1)
        c_foul_h = st.number_input("Falli Casa", 0.0, 30.0, 11.5, 0.5); c_foul_a = st.number_input("Falli Ospite", 0.0, 30.0, 12.5, 0.5)
...
with st.expander("🔥 SCANNER GIORNALIERO: TOP 5 VALUE BETS", expanded=False):
    ...
    if st.button("🔍 Cerca Top 5 Value Bets Ora", type="primary"):
        with st.spinner(f"Elaborazione in corso..."):
            ...
            else: st.warning("Nessuna Value Bet significativa trovata.")
st.divider()
...
if ALL_LEAGUE_ODDS:
    ...
    auto_h_name = TEAM_LIST[0] if TEAM_LIST else None; auto_a_name = TEAM_LIST[1] if len(TEAM_LIST)>1 else None
...
if st.button("🚀 ANALIZZA", type="primary", use_container_width=True):
    with st.spinner("Calcolo Algoritmo ML..."):
        ...
        st.session_state.update({
            ...
            "stats": {"corners": {"1": corn_1, "X": corn_X, "2": corn_2, "lines": corn_lines, "xH": xCorn_H, "xA": xCorn_A}}
        })
...
if st.session_state.analyzed:
    st.markdown("---")
    if st.session_state.ml_active: st.success("🤖 Multi-Target AI Attiva!")
    ...
    tab1, tab2, tab3, tab4, tab5 = st.tabs(["🏆 Esito", "⚽ Goal", "⛳ Extra", "📝 Storico & Backup", "⚡ Combo"])
    ...
    with tab4:
        ...
        if st.button("💾 Salva Match Corrente nel Database"):
            ...
            st.success("Dati tattici salvati! Ricordati di scaricare il Backup se chiudi l'app.")
            
        st.divider()
        st.markdown("#### 🔄 Salvataggio di Sicurezza (Backup File)")
        st.warning("⚠️ Streamlit cancella i dati quando l'app si addormenta. Scarica il file JSON a fine giornata e ricaricalo qui la volta successiva per non perdere l'addestramento.")
        ...
        if st.session_state.history:
            for i, match in enumerate(reversed(st.session_state.history[-15:])):
                ...
                # 🟢 IL CAMPO DI TESTO PER IL RISULTATO ESATTO "2-1"
