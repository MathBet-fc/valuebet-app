...
# w_form: Peso della Forma recente (Ultime 5)
# w_seas: Peso dei dati Stagionali
LEAGUES = {
    ...
    "🇮🇹 Serie B":         {"avg": 1.15, "ha": 85,  "w_elo": 0.20, "w_seas": 0.30, "w_form": 0.50}, # La forma conta di più
    "🇬🇧 Premier League":  {"avg": 1.48, "ha": 105, "w_elo": 0.45, "w_seas": 0.35, "w_form": 0.20}, # Elo molto stabile
    "🇩🇪 Bundesliga":      {"avg": 1.58, "ha": 105, "w_elo": 0.35, "w_seas": 0.40, "w_form": 0.25},
    "🇪🇸 La Liga":         {"avg": 1.25, "ha": 100, "w_elo": 0.50, "w_seas": 0.30, "w_form": 0.20}, # Le big vincono spesso
...
def calculate_complex_xg_dynamic(elo_h, elo_a, att_h, def_h, att_a, def_a, current_avg, current_ha, w_elo_l):
...
st.markdown("ℹ️ *Algoritmo Ibrido: Monte Carlo (10k Sim) + Poisson + Pesi Dinamici*")
...
with st.expander("🔗 Link Utili (Clicca per aprire)", expanded=False):
    ...
    with lc1:
        ...
        st.link_button("StatsSulCalcio", "https://www.statistichesulcalcio.com/classifiche_elo.php")
    with lc2:
        st.caption("Stats")
        st.link_button("FootyStats", "https://footystats.org/it/italy/serie-a/form-table")
...
with col_h:
    ...
    raw_h_elo = st.number_input("Rating (Elo/Opta)", value=1500.0, step=10.0, key="h_elo")
...
with col_a:
    ...
    raw_a_elo = st.number_input("Rating (Elo/Opta)", value=1450.0, step=10.0, key="a_elo")
...
# --- CORE LOGIC ---
if st.button("🚀 ANALIZZA PARTITA (Simulazione Avanzata)", type="primary", use_container_width=True):
    
    # 1. Calcolo Pesi Dinamici
    h_avg_gf_form = h_gf_l5 / 5.0
    h_avg_gs_form = h_gs_l5 / 5.0
    # Formula con Pesi Dinamici
    att_h = (h_gf_seas * W_SEASON) + (h_avg_gf_form * W_FORM) + (h_gf_venue * W_VENUE)
    def_h = (h_gs_seas * W_SEASON) + (h_avg_gs_form * W_FORM) + (h_gs_venue * W_VENUE)
    
    a_avg_gf_form = a_gf_l5 / 5.0
    a_avg_gs_form = a_gs_l5 / 5.0
    att_a = (a_gf_seas * W_SEASON) + (a_avg_gf_form * W_FORM) + (a_gf_venue * W_VENUE)
    def_a = (a_gs_seas * W_SEASON) + (a_avg_gs_form * W_FORM) + (a_gs_venue * W_VENUE)

    xg_h, xg_a = calculate_complex_xg_dynamic(h_elo, a_elo, att_h, def_h, att_a, def_a, CURRENT_AVG_GOALS, CURRENT_HOME_ADV, W_ELO)
    ...
    uo_probs_math = {0.5: [0,0], 1.5: [0,0], 2.5: [0,0]}
    RHO = -0.13 
    ...
    mg_1_2, mg_1_3, mg_2_3, mg_2_4, mg_3_5 = get_multigol_prob(1,2), get_multigol_prob(1,3), get_multigol_prob(2,3), get_multigol_prob(2,4), get_multigol_prob(3,5)
    ...
    col_xg2.info(f"🎲 **Check Monte Carlo (10k Sim):**\n1: {sim_1:.1%} | X: {sim_X:.1%} | 2: {sim_2:.1%}")
    ...
    with col_main:
        ...
        e1, eX, e2 = ((b1/f1)-1)*100, ((bX/fX)-1)*100, ((b2/f2)-1)*100
    ...
    with col_dc:
        ...
        f1X, fX2, f12 = (1/p1X if p1X>0 else 0), (1/pX2 if pX2>0 else 0), (1/p12 if p12>0 else 0)
        data_dc = {
            "Esito": ["1X", "X2", "12"],
    ...
    for i, item in enumerate(top_3_scores):
        ...
        with cols_cs[i]:
            st.metric(f"Top {i+1}: {item['score']}", f"{item['prob']*100:.1f}%", f"Fair: {fair:.2f}")

    # Heatmap
    with st.expander("🗺️ Apri Mappa Probabilità (Heatmap)"):
        fig, ax = plt.subplots(figsize=(6, 5))
        sns.heatmap(score_matrix, annot=True, fmt=".1%", cmap="YlGnBu", 
                    xticklabels=range(6), yticklabels=range(6), cbar=False)
    ...
    with col_uo:
        ...
        for line in [0.5, 1.5, 2.5]:
            ...
            h_stat_o = h_uo.get(line, 90) / 100.0
            a_stat_o = a_uo.get(line, 90) / 100.0
            avg_hist_over = (h_stat_o + a_stat_o) / 2.0
            o_final = (o_math * (1 - WEIGHT_HISTORICAL_UO)) + (avg_hist_over * WEIGHT_HISTORICAL_UO)
            u_final = 1.0 - o_final
            f_u = 1/u_final if u_final > 0 else 0
            f_o = 1/o_final if o_final > 0 else 0
            uo_data.append({"Linea": line, "U %": f"{u_final*100:.1f}%", "Fair U": f"{f_u:.2f}", "O %": f"{o_final*100:.1f}%", "Fair O": f"{f_o:.2f}"})
        st.dataframe(pd.DataFrame(uo_data), hide_index=True)

    with col_gg:
        st.subheader("⚽ Gol / No Gol")
        f_gg, f_ng = (1/p_gg if p_gg>0 else 0), (1/p_ng if p_ng>0 else 0)
        e_gg, e_ng = ((bGG/f_gg)-1)*100 if f_gg>0 else -100, ((bNG/f_ng)-1)*100 if f_ng>0 else -100
        data_gg = {
            "Esito": ["GOL", "NO GOL"],
    ...
    mg_data = [
        {"Range": "1-2", "Prob": mg_1_2, "Fair": 1/mg_1_2},
        {"Range": "1-3", "Prob": mg_1_3, "Fair": 1/mg_1_3},
        {"Range": "2-3", "Prob": mg_2_3, "Fair": 1/mg_2_3},
        {"Range": "2-4", "Prob": mg_2_4, "Fair": 1/mg_2_4},
        {"Range": "3-5", "Prob": mg_3_5, "Fair": 1/mg_3_5},
    ]
    # Ordina per probabilità
    mg_data.sort(key=lambda x: x["Prob"], reverse=True)
    # Formattazione per tabella
    mg_display = [{"Range": m["Range"], "Prob %": f"{m['Prob']*100:.1f}%", "Fair Odd": f"{m['Fair']:.2f}"} for m in mg_data]
    st.dataframe(pd.DataFrame(mg_display).head(3), hide_index=True, use_container_width=True)
    ...
    with st.expander("Apri Calcolatore", expanded=False):
        ...
        with c2:
            ...
            val_90 = st.number_input(f"x{'G' if 'GOL' in type_s else 'A'}/90", 0.0, 2.0, 0.40)
        ...
        prob_p1, prob_p2, _ = calculate_player_probability(val_90, mins, ctx_xg, ctx_avg)
        ...
        c_res3.metric("Valore", f"{edge_p:+.1f}%", delta_color="normal" if edge_p<0 else "inverse")

    # CSV
    csv_row = {"Date": datetime.now().strftime("%Y-%m-%d"), "Home": h_name, "Away": a_name, "xG_H": f"{xg_h:.2f}", "xG_A": f"{xg_a:.2f}", "Fair_1": f"{f1:.2f}", "Fair_X": f"{fX:.2f}", "Fair_2": f"{f2:.2f}", "Top_Score": f"{top_3_scores[0]['score']}"}
    st.download_button("💾 CSV", pd.DataFrame([csv_row]).to_csv(index=False).encode('utf-8'), f"bet_{h_name}_{a_name}.csv", "text/csv")
...
with st.expander("Apri Analizzatore Quote Inverse", expanded=False):
    ...
    with col_rev1:
        rev_quota = st.number_input("Quota Bookmaker (es. 1.90)", 1.01, 100.0, 1.90)
    ...
    st.metric("Probabilità Implicita (senza aggio)", f"{implied_prob*100:.1f}%")
    
    if implied_prob > 0.60:
        st.warning("Il bookmaker vede una vittoria NETTA. Serve una differenza xG > 1.0")
    elif implied_prob > 0.45:
        st.info("Il bookmaker vede un vantaggio SOLIDO. Serve una differenza xG tra 0.4 e 0.8")
    elif implied_prob > 0.35:
        st.write("Quota equilibrata/incerta.")
    else:
        st.write("Il bookmaker vede questo esito come sfavorito/poco probabile.")
