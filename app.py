        ...
        with st.spinner("🔍 Il Figghiozzo sta forzando i sistemi anti-bot per estrarre le formazioni..."):
            try:
                ...
                try:
                    ...
                    try:
                        search_results = list(DDGS().news(query_news, region='it-it', timelimit='w', max_results=4))
                        if not search_results: search_results = list(DDGS().text(query_news, region='it-it', timelimit='w', max_results=4))
                    except Exception: pass
                    
                    if not search_results:
                        piano_c_attivo = True
                        try:
                            search_results = list(DDGS().text(f"{h_name} tabellino formazioni", region='it-it', timelimit='m', max_results=2))
                            search_results += list(DDGS().text(f"{a_name} tabellino formazioni", region='it-it', timelimit='m', max_results=2))
                        except Exception: pass
                        
                    # FALLBACK BING STRUTTURALE (Anti-Ratelimit/Cloudflare)
                    if not search_results:
                        try:
                            url = f"https://www.bing.com/search?q={query_news.replace(' ', '+')}"
                            res_http = scraper.get(url, headers={"User-Agent": "Mozilla/5.0 (Windows NT 10.0; Win64; x64)"}, timeout=7)
                            soup_bing = BeautifulSoup(res_http.text, 'html.parser')
                            for li in soup_bing.find_all('li', class_='b_algo', limit=4):
                                a_tag = li.find('a'); p_tag = li.find('p')
                                if a_tag and p_tag:
                                    search_results.append({"href": a_tag.get('href'), "title": a_tag.text, "body": p_tag.text})
                        except Exception: pass
                ...
                if "429" in str(e) or "Quota" in str(e):
                    st.error("Errore 429: Quota chiamate gratuite Gemini esaurita. Riprova tra 1 minuto.")
                else:
                ...
                with st.spinner("🔍 Il Figghiozzo sta cercando notizie, estraendo le Combo e studiando i giocatori..."):
                    try:
                        ...
                        try:
                            search_results = []
                            try:
                                search_results = list(DDGS().news(search_query, region='it-it', timelimit='w', max_results=3))
                            except Exception: pass
                            
                            if not search_results:
                                try:
                                    search_query_2 = f"{st.session_state.h_name} infortunati squalificati out"
                                    search_results = list(DDGS().text(search_query_2, region='it-it', timelimit='m', max_results=2))
                                    search_query_3 = f"{st.session_state.a_name} infortunati squalificati out"
                                    search_results += list(DDGS().text(search_query_3, region='it-it', timelimit='m', max_results=2))
                                except Exception: pass
                        ...
                        # 3. CONFIGURA GEMINI
                        client = genai.Client(api_key=GEMINI_API_KEY)
                        # Utilizzeremo "gemini-2.5-flash"
                        modello_valido = "gemini-2.5-flash"
                        ...
                        chat_session = client.chats.create(model=modello_valido)
                        st.session_state.figghiozzo_chat_session = chat_session
                        ...
                        st.error("🚨 Mancano le librerie. Aggiungi 'beautifulsoup4' e 'ddgs' al file requirements.txt e fai un Reboot.")
                    except Exception as e:
                        if "429" in str(e) or "Quota" in str(e):
                            st.error("Errore 429: Quota chiamate gratuite Gemini esaurita. Riprova tra 1 minuto.")
                        else:
            ...
            if user_input := st.chat_input("Fai una domanda al Figghiozzo su questa partita..."):
                ...
                with st.spinner("Il Figghiozzo sta digitando..."):
                    try:
                        ...
                        # Recuperiamo la sessione chat se esiste, altrimenti la ricreiamo
                        if "figghiozzo_chat_session" in st.session_state:
                            chat_session = st.session_state.figghiozzo_chat_session
                        else:
                        ...
                        if "429" in str(e) or "Quota" in str(e):
                            st.error("Errore 429: Quota limitata. Attendi.")
                        else:
