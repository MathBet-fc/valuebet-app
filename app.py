...
if st.button("🚀 ANALIZZA", type="primary", use_container_width=True):
    with st.spinner("Calcolo Algoritmo ML Multi-Target..."):
        ...
        # Robustezza: se su Cloud carica una versione vecchia di engine.py (senza infer_max_goals),
        # l'app non deve crashare: usiamo la versione "legacy" 10x10.
        use_dynamic = hasattr(engine, "infer_max_goals") and hasattr(engine, "build_dixon_coles_probabilities")
        if use_dynamic:
            max_goals = engine.infer_max_goals(f_xh, f_xa)
            matrix, p1, pX, p2, pGG, pO25 = engine.build_dixon_coles_probabilities(
                f_xh, f_xa, L_DATA["rho"], max_goals, return_matrix=True
            )

            # Lista risultati esatti (solo range piccolo per UI)
            scores = []
            n = int(matrix.shape[0])
            for h in range(min(6, n)):
                for a in range(min(6, n)):
                    scores.append({"Risultato": f"{h}-{a}", "Prob": float(matrix[h, a])})
        else:
            ...
            for h in range(10):
                for a in range(10):
                    ...
                    if (h + a) > 2.5:
                        pO25 += p
                    if h < 6 and a < 6:
                        scores.append({"Risultato": f"{h}-{a}", "Prob": float(p)})
        ...
        mc_out = engine.monte_carlo_simulation(f_xh, f_xa)
        # compatibilita' con versioni vecchie di engine (che ritornavano una lista)
        if isinstance(mc_out, tuple) and len(mc_out) == 2:
            counts, n_sims = mc_out
        else:
            sim = mc_out
            n_sims = len(sim) if sim is not None else 0
            counts = [sim.count(1), sim.count(0), sim.count(2)] if n_sims else [0, 0, 0]
        ...
        stability = max(
            ...
            - (
                (
                    abs(p1 - (counts[1] / n_sims))
                    + abs(pX - (counts[0] / n_sims))
                    + abs(p2 - (counts[2] / n_sims))
                )
                / 3
                * 400
            ),
...
if st.session_state.analyzed:
    ...
    with tab1:
        ...
        with c_2:
            ...
            ph_minus_15 = np.sum(matrix[idx_h - idx_a >= 2])
            pa_plus_15 = 1 - ph_minus_15
            pa_minus_15 = np.sum(matrix[idx_a - idx_h >= 2])
        ...
        with c1:
            ...
            for l in [0.5, 1.5, 2.5, 3.5, 4.5]:
                if l == 2.5: 
                    ...
                    p_pure = np.sum(matrix[idx_sum > l])
    ...
    with tab3:
        ...
        with cm1:
            ...
            for r in [(1,2), (1,3), (2,3), (2,4), (3,4), (3,5)]:
                pm = np.sum(matrix[(idx_sum >= r[0]) & (idx_sum <= r[1])])
        ...
        with cm2:
            ...
            for r in [(1,2), (1,3), (2,3), (2,4)]:
                pm = np.sum(matrix[(idx_h >= r[0]) & (idx_h <= r[1])])
        ...
        with cm3:
            ...
            mga_res = []
            for r in [(1,2), (1,3), (2,3), (2,4)]:
                pm = np.sum(matrix[(idx_a >= r[0]) & (idx_a <= r[1])])
