...
# Import opzionali (richiedono pacchetti extra) - gestiti con fallback
try:
...
# get_blended_val è ora in engine.py — usiamo direttamente engine.get_blended_val()
...
with col_h:
    ...
    with st.expander("📊 Dati (Mix Stagione/Trend)", expanded=True):
        def_att_s = engine.get_blended_val(h_stats, 'gf', data_mode, w_seas) if h_stats else 1.85
        def_def_s = engine.get_blended_val(h_stats, 'gs', data_mode, w_seas) if h_stats else 0.95
        c1, c2 = st.columns(2)
        ...
        c3, c4 = st.columns(2)
        h_att_home = c3.number_input("Attacco Casa", 0.0, 5.0, float(engine.get_blended_val({"total": h_stats["home"], "form": h_stats["home"]}, 'gf', data_mode, 1.0) if h_stats else def_att_s*1.15), 0.01)
        h_def_home = c4.number_input("Difesa Casa", 0.0, 5.0, float(engine.get_blended_val({"total": h_stats["home"], "form": h_stats["home"]}, 'gs', data_mode, 1.0) if h_stats else def_def_s*0.85), 0.01)
...
with col_a:
    ...
    with st.expander("📊 Dati (Mix Stagione/Trend)", expanded=True):
        def_att_s_a = engine.get_blended_val(a_stats, 'gf', data_mode, w_seas) if a_stats else 1.45
        def_def_s_a = engine.get_blended_val(a_stats, 'gs', data_mode, w_seas) if a_stats else 0.85
        c5, c6 = st.columns(2)
        ...
        c7, c8 = st.columns(2)
        a_att_away = c7.number_input("Attacco Fuori", 0.0, 5.0, float(engine.get_blended_val({"total": a_stats["away"], "form": a_stats["away"]}, 'gf', data_mode, 1.0) if a_stats else def_att_s_a*0.85), 0.01)
        a_def_away = c8.number_input("Difesa Fuori", 0.0, 5.0, float(engine.get_blended_val({"total": a_stats["away"], "form": a_stats["away"]}, 'gs', data_mode, 1.0) if a_stats else def_def_s_a*1.15), 0.01)
    with st.expander("Over Trend"):
...
if st.button("📰 Cerca News e Avvia Pre-Analisi", type="secondary"):
    ...
    elif not (_DDGS_OK and _CLOUDSCRAPER_OK and _BS4_OK and _GENAI_OK):
        st.error("🚨 Mancano librerie necessarie. Aggiungi 'duckduckgo-search', 'cloudscraper', 'beautifulsoup4', 'google-generativeai' al requirements.txt e fai un Reboot.")
    else:
        with st.spinner("🔍 Il Figghiozzo sta forzando i sistemi anti-bot per estrarre le formazioni..."):
            try:
                pass  # import già in cima al file
...
if st.button("🚀 ANALIZZA", type="primary", use_container_width=True):
    with st.spinner("Calcolo Algoritmo ML Multi-Target..."):
        # Calcolo xG aggiustati tramite funzione condivisa in engine.py
        f_xh, f_xa, h_ppda, a_ppda, h_dc, a_dc = engine.calculate_match_xg(
            h_stats, a_stats, L_DATA, m_type, h_elo, a_elo, w_seas, data_mode,
            h_att=h_att, h_def=h_def, a_att=a_att, a_def=a_def,
            h_att_split=h_att_home, h_def_split=h_def_home,
            a_att_split=a_att_away, a_def_split=a_def_away,
            volatility=volatility, h_str=h_str, a_str=a_str,
            h_rest=h_rest, a_rest=a_rest, is_big_match=is_big_match,
            h_m_a=h_m_a, h_m_d=h_m_d, a_m_a=a_m_a, a_m_d=a_m_d
        )
        ...
        if "figghiozzo_chat" in st.session_state and len(st.session_state.figghiozzo_chat) > 0:
            ...
            if user_input := st.chat_input("Fai una domanda al Figghiozzo su questa partita..."):
                ...
                with st.spinner("Il Figghiozzo sta digitando..."):
                    try:
                        genai.configure(api_key=GEMINI_API_KEY)
                        # Seleziona il modello (con inizializzazione esplicita per evitare UnboundLocalError)
                        modello_valido = None
                        for m in genai.list_models():
                            if 'generateContent' in m.supported_generation_methods and 'flash' in m.name:
                                ...
                                break
                        if not modello_valido:
                            st.error("❌ Nessun modello Gemini compatibile trovato.")
                            st.stop()
                        model = genai.GenerativeModel(modello_valido)
